...
const Flights = () => {
  ...
  const Interest = [
    {
      id: '1', Image: Gulberg, title: 'Magnova Studios', location: 'Poland, Kiszkowo',
      rating: '9.7', reviews: '630 reviews', start: 'Starting from', priceNew: 'PKR 51,570',
    },

    {
      id: 2, Image: Das, title: 'Magnova Studios', location: 'Poland, Kiszkowo',
      rating: '9.7', reviews: '630 reviews', start: 'Starting from', priceNew: 'PKR 51,570',
    }
  ]
        ...
        <Box>
          ...
          <Grid container spacing={1}>
            {Interest.map((interest) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={interest.id}>
                <Card className='py-2 mt-3' sx={{ borderRadius: 2, boxShadow: '0 4px 10px rgba(0, 0, 0, 0.1)', overflow: 'hidden', }}>
                  <CardMedia component="img" image={interest.Image} alt={Interest} sx={{ height: 200, objectFit: 'cover' }} />
                  <Typography variant="body2" className="fs-5 fw-bolder p-2"> {interest.title} </Typography>
                  <Typography variant="body2" className="ms-2" color="textSecondary">{interest.location}
                  </Typography>
                  <Typography className="ps-1" sx={{
                    fontSize: '0.9rem', fontWeight: 'bold', display: 'flex',
                    alignItems: 'center', gap: 1
                  }}>
                    <Chip label={interest.rating} color="primary" size="small" sx={{ fontWeight: 'bold' }} />
                    <Typography variant="body1" fontWeight="bold" className="p-2"> {Interest.reviews}</Typography>
                  </Typography>
